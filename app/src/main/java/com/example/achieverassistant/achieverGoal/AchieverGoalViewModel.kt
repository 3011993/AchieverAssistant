...
class AchieverGoalViewModel(private val database: AchieverGoalDatabase, application: Application) : AndroidViewModel(application) {
    ...
    fun insertStep(steps: Steps) {
        viewModelScope.launch(Dispatchers.IO) {
            database.stepsDao().insertStep(steps)
        }
    }

    fun deleteStep(steps: Steps) {
        viewModelScope.launch(Dispatchers.IO) {
            database.stepsDao().deleteStep(steps)
        }
    }

    fun updateStep(steps: Steps) {
        viewModelScope.launch(Dispatchers.IO){
            database.stepsDao().updateStep(steps)
        }
    }
    ...
    private  fun getAllSteps(){
        viewModelScope.launch(Dispatchers.IO){
            database.stepsDao().getAllSteps().collect{
    ...
    class AchieverGoalFactory(val database: AchieverGoalDatabase, val app: Application) : ViewModelProvider.Factory {
